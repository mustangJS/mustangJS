...
/*
	*-------------------------------------------------------*
	...
	|-------------------------------------------------------*
	|
	| An API to the DOM created by Eric Diviney. Made for 
	| extremely fast DOM access & manipulation. Dependency for 
	| Mustang.js is Sizzle.js selector engine, and Watch.js 
...
*/
	...
	*/
	...
	CollectionRelationship.prototype.toJson = function()
	{
		...
		{
			if(typeof this.elems[i].elem.name == "undefined")
			{
				tmp["value" + i] = this.rel[i].model;
			}
			else
			{
				tmp[this.elems[i].elem.name] = this.rel[i].model;
			}
	...
	*/
	...
	/*
		FormRelationship bindForm

		 * @var (none)
		 *
		 * @return FormRelationship object

		This is a special function only available to a Mustang object representing an entire form element
		Traverses through each input of the form (excluding submit and buttons and reset input) & binds them
		to one large object, which is then 'watched'. toJson() will make forms USER SIMPLE
	...
	*/
	...
	MustangCollection.prototype.bindForm = function(vals)
	{
...
// Mustang to do
	//  Bind entire form to object
	// add methods to cods type(), click(), addClass(), & removeClass()


// MustangCollection to do
	//  onclick for entire Mustang Collection
	//  Bind multiple elements to an object
	//  Bind multiple elements to an array
	//  .even() and .odd() functions of collections which returns correct elements
	//  .foreach() on mustang collections
	//  .BindToArr() for mustang collection inputs
	// bindAll() and 
	// add class and remove class methods

// CollectionRelationship
	

// Mustang Library To-Do's
	//This framework not meant for large amounts of logic and server calls, it's meant for quick dom manipulation
	//Stallion.js can be used for server calls 
	//Colt.js for something else
	//Bronco.js
	// toJson for both Mustang, Mustang Collection, and Relationship classes
